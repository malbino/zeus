 ...
 * @author tincho
 */
@WebServlet(name = "FacturaTramCert", urlPatterns = {"/reportes/FacturaTramCert"})
public class FacturaTramCert extends HttpServlet {
    ...
    @EJB
    EstudianteFacade estudanteFacade;
    ...
    public void generarPDF(HttpServletRequest request, HttpServletResponse response) {
        ...
        Integer id_estudiante = (Integer) request.getSession().getAttribute("id_estudiante");

        if (id_factura != null && id_carrera != null && id_estudiante != null) {
            Factura factura = facturaFacade.find(id_factura);
            Carrera carrera = carreraFacade.find(id_carrera);
            Estudiante estudiante = estudanteFacade.find(id_estudiante);
            ...
            try {
                ...
                document.add(titulo_factura(factura, carrera, estudiante));
                ...
                document.add(titulo_comprobante(factura, carrera, estudiante));
    ...
    public PdfPTable titulo_factura(Factura factura, Carrera carrera, Estudiante estudiante) {
        PdfPTable table = new PdfPTable(1);
        ...
        cell = new PdfPCell(new Phrase("Estudiante: " + estudiante.toString(), normal));
        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
        cell.setBorder(Rectangle.NO_BORDER);
        table.addCell(cell);

        cell = new PdfPCell(new Phrase("Carrera: " + carrera.getNombre(), normal));
        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
        cell.setBorder(Rectangle.BOTTOM);
        cell.setBorderWidthBottom(0.8f);
        cell.setPaddingBottom(10);
    ...
    public PdfPTable titulo_comprobante(Factura factura, Carrera carrera, Estudiante estudiante) {
        PdfPTable table = new PdfPTable(1);
