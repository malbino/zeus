 ...
 * @author tincho
 */
@WebServlet(name = "FacturaFormacion", urlPatterns = {"/reportes/FacturaFormacion"})
public class FacturaFormacion extends HttpServlet {
    ...
    public void generarPDF(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (id_factura != null && id_inscrito != null) {
            ...
            try {
                ...
                float largo = ALTO_MINIMO + (ALTO_FILA * factura.getDetalles().size());
                ...
                document.add(encabezado(factura));
                document.add(titulo(factura, inscrito));
                document.add(detalle(factura));
                document.add(pie(factura));

                document.newPage();

                document.add(encabezado_coprobante(factura));
                document.add(titulo_comprobante(factura, inscrito));
    ...
    public PdfPTable encabezado(Factura factura) throws BadElementException, IOException {
    ...
    public PdfPTable titulo(Factura factura, Inscrito inscrito) {
        PdfPTable table = new PdfPTable(1);
        ...
        cell = new PdfPCell(new Phrase("Estudiante: " + inscrito.getEstudiante().toString(), normal));
        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
        cell.setBorder(Rectangle.NO_BORDER);
        table.addCell(cell);

        cell = new PdfPCell(new Phrase("Carrera: " + inscrito.getCarrera().getNombre(), normal));
        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
        cell.setBorder(Rectangle.BOTTOM);
        cell.setBorderWidthBottom(0.8f);
        cell.setPaddingBottom(10);
    ...
    public PdfPTable detalle(Factura factura) {
        ...
        List<Detalle> detalles = detalleFacade.listaDetalles_Formacion(factura.getId_factura());
    ...
    public PdfPTable pie(Factura factura) throws BadElementException {
        PdfPTable table = new PdfPTable(1);
    ...
    public PdfPTable titulo_comprobante(Factura factura, Inscrito inscrito) {
        PdfPTable table = new PdfPTable(1);
