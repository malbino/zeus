 ...
 * @author tincho
 */
@WebServlet(name = "FacturaCapacitacion", urlPatterns = {"/reportes/FacturaCapacitacion"})
public class FacturaCapacitacion extends HttpServlet {
    ...
    @EJB
    InscritoPhFacade inscritoFacade;
    ...
    public void generarPDF(HttpServletRequest request, HttpServletResponse response) {
        ...
        Integer id_inscritoph = (Integer) request.getSession().getAttribute("id_inscritoph");

        if (id_factura != null && id_inscritoph != null) {
            Factura factura = facturaFacade.find(id_factura);
            InscritoPh inscritoPh = inscritoFacade.find(id_inscritoph);
            ...
            try {
                ...
                document.add(titulo_factura(factura, inscritoPh));
                ...
                document.add(titulo_comprobante(factura, inscritoPh));
    ...
    public PdfPTable titulo_factura(Factura factura, InscritoPh inscritoPh) {
        PdfPTable table = new PdfPTable(1);
        ...
        cell = new PdfPCell(new Phrase("Estudiante: " + inscritoPh.getEstudiante().toString(), normal));
        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
        cell.setBorder(Rectangle.NO_BORDER);
        table.addCell(cell);

        cell = new PdfPCell(new Phrase("Curso: " + inscritoPh.getGrupo().getCurso().getNombre(), normal));
        cell.setHorizontalAlignment(PdfPCell.ALIGN_LEFT);
        cell.setBorder(Rectangle.BOTTOM);
        cell.setBorderWidthBottom(0.8f);
        cell.setPaddingBottom(10);
    ...
    public PdfPTable titulo_comprobante(Factura factura, InscritoPh inscritoPh) {
        PdfPTable table = new PdfPTable(1);
